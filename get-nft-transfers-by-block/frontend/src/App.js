...
function App() {
  ...
  const refreshNftTransfers = async () => {
    await axios
      .get(
        `/get_transfers?block_number=${params.block_number}&chain=${params.chain}&limit=${params.limit}`
      )
      .then((res) => {
    ...
    Object.values(nftTransfers).map((transfer) => {
      return (
        <Card
          ...
          title={`block number: ${transfer.block_number}`}
          extra={
            <p>
              <b>Log Index:</b> {transfer.log_index}
            </p>
          }
          style={{ width: 300 }}
        >
