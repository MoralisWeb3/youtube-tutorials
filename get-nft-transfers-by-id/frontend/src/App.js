...
function App() {
  ...
  const refreshNftTransfers = async () => {
    await axios
      .get(
        `/get_transfers?chain=${params.chain}&address=${params.address}&token_id=${params.token_id}`
      )
      .then((res) => {
  ...
  return (
    <div>
      <div className={styles.header}>
        ...
        <h1 className={styles.title}>Get NFT Transfers by id </h1>
      </div>
      <section className={styles.main}>
        ...
        <div className={styles.getTokenForm}>
          <label className={styles.label} htmlFor="token_id">
            Token id
          </label>
          <input
            className={styles.walletAddress}
